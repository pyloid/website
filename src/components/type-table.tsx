  ...
  return (
    ...
    >
      <CollapsibleTrigger className='relative flex flex-row items-center w-full group text-start px-3 py-2 not-prose hover:bg-fd-accent'>
        <div className='flex items-center gap-2 min-w-fit w-[50%]'>
          <code
            className={cn(
              keyVariants({
                deprecated,
                className: cn(
                  'font-bold',
                  !required && 'text-fd-muted-foreground'
                ),
              })
            )}
          >
          ...
          {!required && (
            <span className='px-1.5 py-0.5 text-xs bg-fd-background text-fd-muted-foreground rounded-md font-normal'>
              Optional
            </span>
          )}
        </div>
        ...
        <div className='grid grid-cols-[1fr_3fr] gap-y-4 text-sm p-3 overflow-auto fd-scroll-container border-t'>
          ...
          {(typeDescription || type) && (
            <>
              ...
              <p className='my-auto not-prose'>{typeDescription || type}</p>
            </>
          )}
          {defaultValue && (
            <>
